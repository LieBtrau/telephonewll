 ...
 *rising and falling edges of PCLK.
 *The amount of available processing cycles to shift out/in the PCM-data is quite limited.  Shifting out a bit and 
 *shifting in a bit in a single ISR-call takes too long.  So these two operations each have their own ISR's.
 *First the ISR for shifting in data gets called until all data is shifted in.  After that the ISR for shifting out data
 *is called until all data is shifted out on the bus.
 *Rising edge PCLK interrupt is used to shift in data.  Due to ISR-latency, DTX is read on falling edge of PCLK
 *Falling edge PCLK interrupt is used to shift out data.  Due to ISR-latency, DRX is being set near the rising edge of PCLK.
 *
...
//Interrupt routine for shifting in data from SLIC
ISR(TIMER2_COMPA_vect){
  static byte yBitCtr=0;
  PORTD|=0x40;//debug
  ...
  yBitCtr++;
  if(yBitCtr==10){
    bitClear(TIMSK2,OCIE2A);        //Stop shifting in data (disable this ISR)
    bitSet(TIMSK2,OCIE2B);          //Start shifting out data (enable ISR for shifting out)
    yBitCtr=0;
  }
  PORTD&=0xBF;//debug
}

//Interrupt routine for shifting out data to SLIC
ISR(TIMER2_COMPB_vect){
  ...
  yBitCtr++;
  if(yBitCtr==9){                  //Added one extra ISR-call to make DRX low between databytes.
